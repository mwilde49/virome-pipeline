...
- **Bracken hard-fails on an all-unclassified Kraken2 report**: a sample whose STAR-unmapped read pool is tiny and/or entirely non-viral can produce a Kraken2 report containing only the `unclassified` summary line (no rank rows at all) — a legitimate zero-detection result, not a broken input. Real example: `104T8R` (Thoracic DRG cohort, 2026-08-19) had only 33 STAR-unmapped reads total, 100% unclassified. Bracken hard-errors on this ("Error: no reads found. Please check your Kraken report", exit 1), which by Nextflow's default `errorStrategy` aborts the *entire* cohort run, not just that one sample. Fixed in `modules/bracken.nf`: before invoking `bracken`, check whether the report has any row at `params.bracken_level`; if not, skip bracken and pass the original report through untouched as `.bracken_report` (downstream `filter_kraken2_report.py` already handles zero viral-rank rows cleanly — empty DataFrame, 0 taxa, no crash) plus an empty-but-correctly-headered `.bracken` file. No container rebuild needed (pure shell logic in the process script, not a baked-in script). No `errorStrategy`/resilience change made beyond this specific case — one bad sample can still abort a whole cohort for any *other* process failure. **Correction, same run**: `104T8R` is the *only* sample in this cohort that actually triggered the new guard — 92 total STAR input reads, a genuine library failure (same class as `Saad_2`). The other four samples that showed up as fresh `BRACKEN COMPLETED` in the same trace (`101T5L`, `105T8R`, `105T8L`, `106T3R`) were initially mis-read as hitting the same condition; checking their actual `.kraken2.report` files showed all four have real rank-`S` rows and completely normal output — they simply hadn't reached their `BRACKEN` step in any prior run (every earlier attempt aborted on some *other* sample first), so this run was just the first time the DAG scheduler got to them. `106T3R` does have its own separate, genuine anomaly worth noting: 74.52% of its 62.8M reads are "too many loci" (multi-mapping rejected by `--outFilterMultimapNmax 1`), driving its low 24.68% unique-mapping rate — unrelated to the Bracken fix, and not obviously a problem (its resulting ~47M-read unmapped pool classified normally), but unusual enough to flag.
